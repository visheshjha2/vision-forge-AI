...
serve(async (req) => {
  ...
  try {
    ...
    const { sourceImage, enhancePrompt, style } = await req.json();
    ...
    const instruction = `${style ? style + ' style. ' : ''}${enhancePrompt || 'Enhance this image, improve quality and details.'}`;
    ...
    const { error: insertError } = await supabase.from("projects").insert({
      user_id: user.id,
      type: "enhancement",
      prompt: enhancePrompt || "Image enhancement",
      status: "completed",
